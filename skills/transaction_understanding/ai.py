...
def batch_classify_transactions(transactions: List[dict], api_key: str) -> dict:
    ...
    system_instruction = (
        ...
        "Provide a granular sub-category. Examples: 'Groceries', 'Restaurant', 'Medicine', 'Rent', 'Flights', 'Streaming', 'SIP', 'Salary', 'Miscellaneous'.\n\n"
        ...
        "PARSE PERSONAL SHORTHAND: If a description contains names/shorthand (e.g., 'Books: Raj', 'Repay of Books: Raj', 'Cold coffee Raj'), identify the underlying ITEM as the merchant (e.g., 'Books', 'Cold coffee').\n\n"
        "REPAYMENT LOGIC:\n"
        "If description includes 'Repay', 'Settlement', or 'Refund':\n"
        "- If Amount > 0: Type = Income, Category = Other.\n"
        "- If Amount < 0: Type = Loan/Debt, Category = Loan/Debt (with sub_category 'Loan Repayment').\n\n"
        "RULES for 'associated_person':\n"
        "If the description contains a human name indicating who the transaction was for or with (e.g., 'Books for Raj', 'Raj book expense'), extract that name (e.g. 'Raj'). Otherwise, set it to null.\n\n"
